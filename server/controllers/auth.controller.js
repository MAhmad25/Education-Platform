...
const sendOTP = async (req, res) => {
      try {
            ...
            res.status(500).json({ message: "There is the problem in sending OTP controller ", error: error.message });
      }
};
...
const createAccount = async (req, res) => {
      try {
            ...
            if (otpProvided.otp !== otp) return res.status(401).json({ message: "OTP does not match !" });
            // Hash the Password
            ...
            res.status(500).json({ message: "There is the problem in Signup controller ", error: error.message });
      }
};
...
const login = async (req, res) => {
      try {
            ...
            res.status(500).json({ message: "There is the problem Login controller ", error: error.message });
      }
};
